#include "mpi_fitness_calculator.h"
...
void VRP::MpiFitnessCalculator::calculateInterval(double* genome, int len, int genomeSize, vector<double>& fitness) {
    int chunk_len = len / numWorkers;
    int pos = 0;
    for (int i = 0; i < numWorkers; i++) {
        //printf("%d -> %d\n", pos, pos + chunk_len);

        if (i == numWorkers - 1) {
            chunk_len += len % numWorkers;
        }
        MPI_Send(&chunk_len, 1, MPI_INT, i + 1, 0, MPI_COMM_WORLD);
        MPI_Send(genome + pos * genomeSize, chunk_len * genomeSize, MPI_DOUBLE, i + 1, 0, MPI_COMM_WORLD);
    ...
    for (int i = 0; i < numWorkers; i++) {
        ...
        MPI_Recv(fitness.data() + pos, chunk_len, MPI_DOUBLE, i + 1, 0, MPI_COMM_WORLD, &status);

        pos += chunk_len;
    }
}

void VRP::MpiFitnessCalculator::calculate(genetic::IndividualArray& individuals, double* fitness) {
...
void VRP::MpiFitnessCalculator::sort(genetic::IndividualArray& individuals, double* fitness) {
...
void VRP::MpiFitnessCalculator::printIndividual(VRP::VRPIndividual* individual, int genomeSize) {
...
double VRP::MpiFitnessCalculator::calculate(double* genome, int genomeSize, bool print) {
...
double VRP::MpiFitnessCalculator::getDistance(pair<int, int> a, pair<int, int> b) {
    //printf("A = %d; %d\n", a.first, a.second);
    //printf("B = %d; %d\n", b.first, b.second);
