...
namespace VRP {
    class MpiFitnessCalculator : public genetic::FitnessCalculator {
    private:
        VRP::Problem* problem;
        int numVehicles;
    ...
    public:
        MpiFitnessCalculator(VRP::Problem* problem, int numVehicles, BestIndividualKeeper* keeper, int numWorkers)
            : problem(problem), numVehicles(numVehicles), keeper(keeper), numWorkers(numWorkers) {}
